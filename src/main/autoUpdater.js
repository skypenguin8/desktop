...
const UPDATER_INTERVAL_IN_MS = 8 * 60 * 60 * 1000; // 8 hours
...
function createUpdaterModal(parentWindow, options) {
    ...
    let updaterURL = (global.isDev ? 'http://localhost:8080' : `file://${app.getAppPath()}`) + '/browser/updater.html'; // TODO: 로컬에서 global.isDev가 필요한지 체크
...
function initialize(appState, mainWindow, notifyOnly = false) {
    ...
    autoUpdater.on('error', (err) => {
        ...
        setTimeout(() => { // TODO: setTimeout가 필요한지 체크
            autoUpdater.checkForUpdates();
        }, UPDATER_INTERVAL_IN_MS);
    });
}
